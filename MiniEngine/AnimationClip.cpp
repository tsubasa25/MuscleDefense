...
void AnimationClip::Load(const char* tkaFilePath)
{
	m_tkaFile = nsMuscle::ResourceBankManager::GetInstance()->GetTkaFromBank(tkaFilePath);
	if (m_tkaFile == nullptr)
	{
		//未登録
		m_tkaFile = new TkaFile;
		m_tkaFile->Load(tkaFilePath);
		nsMuscle::ResourceBankManager::GetInstance()->RegistTkaFileToBank(tkaFilePath, m_tkaFile);
	}
